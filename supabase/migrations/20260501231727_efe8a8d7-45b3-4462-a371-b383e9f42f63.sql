-- 1) create_order: validar shipping_rate por estado + limite de qty
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    -- Limite defensivo: ninguém compra mais de 100 unidades de um item.
    if v_qty > 100 then
  ...
  if p_shipping_id is not null then
    -- Bug fix: validar que a rate selecionada pertence ao estado informado.
    -- Antes, qualquer ID válido era aceito → cliente podia escolher rate
    -- de outro estado mais barato.
    select * into v_rate from public.shipping_rates
      where id = p_shipping_id and active = true;
    if not found then
      v_shipping := 80;
    elsif upper(v_rate.state) <> v_state then
...
-- 2) Trigger: devolver uso do cupom em pedidos cancelados/reembolsados
CREATE OR REPLACE FUNCTION public.refund_coupon_on_order_cancel()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  IF TG_OP = 'UPDATE'
     ...
     AND length(trim(NEW.coupon_code)) > 0 THEN
...
DROP TRIGGER IF EXISTS trg_refund_coupon_on_order_cancel ON public.orders;
CREATE TRIGGER trg_refund_coupon_on_order_cancel
AFTER UPDATE OF status ON public.orders
FOR EACH ROW
EXECUTE FUNCTION public.refund_coupon_on_order_cancel();